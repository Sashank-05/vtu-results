...
def scrape_student():
    ...
    try:
        handler = threadedgetdata.ThreadManager(custom_url, usn_prefix, f"{usn_prefix[1:]}_SEM_{sem}", end_usn,
                                                num_threads=num_threads, socketio=socketio)

        # after defining threads tell the server a message and start the threads without using socketio
